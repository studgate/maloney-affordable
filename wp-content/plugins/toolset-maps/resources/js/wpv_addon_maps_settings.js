...
WPViews.ViewAddonMapsSettings = function( $ ) {
	...
	self.showAppropriateSettingsSections = function( apiUsed ) {
        ...
        else if (apiUsed === API_AZURE) {
			$shownForGoogle.hide();
			$shownForAzure.show();
		}
        else if (apiUsed === API_OSM) {
            // Hide both for OpenStreetMap, which needs no API key
            $shownForGoogle.hide();
            $shownForAzure.hide();
        }
	};
	...
	$( document ).on( 'click', '#js-wpv-maps-check-missing-cache', function() {
		const $button = $( this );
		const $messageSpace = $( 'div.js-wpv-map-load-stored-data-after' );

		$button.prop( 'disabled', true );
		$messageSpace.html( '<div class="wpv-spinner ajax-loader">' ).show();

		$.getJSON( {
			url: ajaxurl,
			data: {
				action: 'wpv_maps_check_missing_cache',
				wpnonce: wpv_addon_maps_settings_local.global_nonce
			}
		} )
		.done( response => $messageSpace.html( response.data ) )
