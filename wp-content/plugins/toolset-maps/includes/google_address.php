...
class Toolset_Addon_Maps_Types {
	...
	function __construct() {
		...
		// Get a Types address field meta key
		add_filter( 'toolset_filter_toolset_maps_get_types_field_meta_key', array( $this, 'get_types_field_meta_key'), 10, 2 );
		// Init
	...
	*/
	...
	public function get_types_field_meta_key( $field_name, $domain = 'postmeta' ) {
		if ( empty( $field_name ) ) {
			return $field_name;
		}
		...
		switch ( $domain ) {
			case 'postmeta':
				$types_fields = self::$types_postmeta_fields;
				break;
			case 'termmeta':
				$types_fields = self::$types_termmeta_fields;
				break;
			case 'usermeta':
				$types_fields = self::$types_usermeta_fields;
				break;
		}
		...
		if (
			array_key_exists( $field_name, $types_fields )
			&& is_array( $types_fields[ $field_name ] )
			&& isset( $types_fields[ $field_name ]['meta_key'] )
		) {
			return $types_fields[ $field_name ]['meta_key'];
		}

		return $this->guess_types_field_meta_key( $original_field_name, $types_fields );
	}

	/**
	 * Solve a problem with distance filters saved in the blocks editor
	 *
	 * @param  string $field_name
	 * @param  array  $types_fields
	 *
	 * @return string
	 */
	private function guess_types_field_meta_key( $field_name, $types_fields ) {
		// Backward compatibility!!!
		// For some reasons, Views filters might store the field name instead of the field meta_key
		// when there is only one field: that field name gets wrongly pushed as the default value.
		// In this case, the field that appears as selected in the editor is the first one and the search filed might store its name instead of its meta_key.
		$maybe_fields = array_values( array_filter(
			$types_fields,
		...
		if (
			count( $maybe_fields ) > 0
			&& isset( $maybe_fields[0]['meta_key'] )
		) {
			return $maybe_fields[0]['meta_key'];
		}
		...
		if (
			...
			&& count( $types_fields ) > 0
		) {
			$types_fields_definitions = array_values( $types_fields );
			if ( isset( $types_fields_definitions[0]['meta_key'] ) ) {
				return $types_fields_definitions[0]['meta_key'];
			}
		}
