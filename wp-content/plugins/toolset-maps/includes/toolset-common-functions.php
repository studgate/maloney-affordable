...
class Toolset_Addon_Maps_Common {
    ...
    const API_OSM = 'osm';
    // Nominatim endpoint (for free GeoCoding and address lookups):
    const API_OSM_URL = 'https://nominatim.openstreetmap.org/search';

	// This can be 50: https://developers.google.com/maps/documentation/geocoding/geocoding-strategies#rate-limits
	// (Of course, that's per second, and our throttle is per request... But, this is the best we can do.)
	const API_CALL_THROTTLE_MAX = 50;
	...
	public static function get_marker_icons() {
		return array(
	...
	function init() {
		...
		add_action( 'elementor/frontend/the_content', array( $this, 'reset_used_ids_elementor') );
		...
		$editor_blocks = new Compatibility\Gutenberg\EditorBlocks\MapsEditorBlocks();
	...
	protected function render_api_select_settings_section() {
        ...
        $osm    = __( 'OpenStreetMap', 'toolset-maps' );

		$api_google = self::API_GOOGLE;
		$api_azure  = self::API_AZURE;
        $api_osm    = self::API_OSM;
        ...
        $api_osm_checked    = checked( $this->get_api_used(), self::API_OSM, false );
		...
		return <<<HTML
			...
			<ul class="js-toolset-maps-api-used-form">
                ...
                <li>
                    <label><input type="radio" name="toolset-maps-api-used" value="$api_osm" $api_osm_checked />$osm</label>
                </li>				
			</ul>
HTML;
	}
...
HTML;
	...
	/**
	 * Elementor sometimes renders content multiple times (and in content templates, this somehow leads to markers
	 * loosing track of their map ids. Anyway, resetting ids per content render in Elementor should be a safe way to
	 * fix this.
	 *
	 * @since 2.0.2
	 *
	 ...
	 * @return string
	 */
	public function reset_used_ids_elementor( $content ) {
		self::$used_map_ids = array();
		self::$used_marker_ids = array();
		return $content;
	}
	...
	function register_assets() {
        ...
        } elseif ( self::API_AZURE === $this->get_api_used() ) {
			$this->register_azure_specific_assets( $assets_url );
        } elseif ( self::API_OSM === $this->get_api_used() ) {
            $this->register_osm_specific_assets( $assets_url );
		}
        ...
        if ( self::get_saved_option( 'api_key') ) {
			...
			if ( self::API_GOOGLE === $this->get_api_used() ) {
				$settings_script_dependencies[] = 'google-maps';
			}
        }
		wp_register_script(
			'views-addon-maps-settings-script',
			TOOLSET_ADDON_MAPS_URL . '/resources/js/wpv_addon_maps_settings.js',
		...
		wp_localize_script(
			'views-addon-maps-settings-script',
			'wpv_addon_maps_settings_local',
			$wpv_addon_maps_settings_localization
		);
	...
	protected function register_google_specific_assets( $assets_url ) {
		...
		wp_register_script(
			...
			array( 'jquery', 'jquery-migrate' ), // google-maps gets lazy loaded when needed.
			'1.7.0',
			true
		);
		...
		wp_register_script(
			...
			array( 'jquery', 'underscore' ), // google-maps gets lazy loaded when needed.
			TOOLSET_ADDON_MAPS_VERSION,
			true
		);
		...
		wp_register_script(
			...
			$assets_url . '/resources/js/markerclusterer.js',
			array( 'jquery', 'underscore', 'views-addon-maps-script' ), // google-maps gets lazy loaded when needed.
			'1.0',
			true
		);
		wp_localize_script(
		...
		wp_register_script(
			...
			array(), // google-maps gets lazy loaded when needed.
			'1.0',
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'add_to_cache_nonce' => wp_create_nonce( Ajax::CALLBACK_ADD_TO_CACHE ),
				'mapStyle'           => self::get_given_or_default_style_json(),
			)
		);
	}

	private function register_azure_specific_assets( $assets_url ) {
		...
		$address_autocomplete_script_dependencies = ['jquery', 'jquery-ui-autocomplete', 'underscore'];
		if ( ! is_admin() ) {
			$address_autocomplete_script_dependencies[] = 'azure-maps-js';
		}
		wp_register_script(
			'toolset-maps-address-autocomplete',
			$assets_url . '/resources/js/toolset_maps_address_autocomplete.js',
			$address_autocomplete_script_dependencies,
			TOOLSET_ADDON_MAPS_VERSION
		);
		wp_localize_script(
			'toolset-maps-address-autocomplete',
		...
		// Frontend rendering script
		$frontend_rendering_script_dependencies = ['jquery', 'underscore'];
		// Skip azure for Gutenberg, it will load lazy when needed.
		if ( ! is_admin() ) {
			$frontend_rendering_script_dependencies[] = 'azure-maps-js';
		}
		wp_register_script(
			'views-addon-maps-script',
			$assets_url . '/resources/js/wpv_addon_maps.js',
			$frontend_rendering_script_dependencies,
			TOOLSET_ADDON_MAPS_VERSION,
			true
		);
	}

    private function register_osm_specific_assets( $assets_url ) {
        ...
        // Register Leaflet.markercluster CSS
        wp_register_style(
            'leaflet-markercluster-css',
            'https://unpkg.com/leaflet.markercluster@1.5.3/dist/MarkerCluster.css',
            array('leaflet-css'), // Ensure it loads after Leaflet CSS
            '1.5.3'
        );

        // Register Leaflet.markercluster Default CSS
        wp_register_style(
            'leaflet-markercluster-default-css',
            'https://unpkg.com/leaflet.markercluster@1.5.3/dist/MarkerCluster.Default.css',
            array('leaflet-css', 'leaflet-markercluster-css'), // Load after MarkerCluster CSS
            '1.5.3'
        );

        // Register Leaflet.markercluster JS
        wp_register_script(
            'leaflet-markercluster-js',
            'https://unpkg.com/leaflet.markercluster@1.5.3/dist/leaflet.markercluster.js',
            array('leaflet-js'), // Ensure it loads after Leaflet JS
            '1.5.3',
            true
        );

        // Register Custom Maps Script
        wp_register_script(
            'views-addon-maps-script',
            $assets_url . '/resources/js/wpv_addon_maps.js',
            array( 'jquery', 'underscore', 'leaflet-js', 'leaflet-markercluster-js' ), // Add 'leaflet-markercluster-js' as a dependency
            TOOLSET_ADDON_MAPS_VERSION,
            true
        );

        // Register Address Autocomplete Script
        wp_register_script(
            'toolset-maps-address-autocomplete',
            $assets_url . '/resources/js/toolset_osm_maps_address_autocomplete.js',
            array( 'jquery', 'jquery-ui-autocomplete', 'underscore', 'leaflet-js', 'leaflet-markercluster-js' ),
            TOOLSET_ADDON_MAPS_VERSION
        );
        wp_localize_script(
            'toolset-maps-address-autocomplete',
            'toolset_maps_address_autocomplete_i10n',
            array(
                'useragent'         => self::get_nominatim_user_agent(),
                'showhidecoords'	=> __( 'Show/Hide coordinates', 'toolset-maps' ),
                'latitude'			=> __( 'Latitude', 'toolset-maps' ),
                'longitude'			=> __( 'Longitude', 'toolset-maps' ),
                'usethisaddress'	=> __( 'Use this address', 'toolset-maps' ),
	...
	function wp_enqueue_scripts() {
        ...
        self::maybe_enqueue_osm_scripts();
	}
    ...
    /**
     * Helper method to potentially enqueue OpenStreetMaps CSS/JS scripts
     * @since 1.5.4
     */
    public static function maybe_enqueue_osm_scripts() {
        if ( apply_filters( 'toolset_maps_get_api_used', '' ) === Toolset_Addon_Maps_Common::API_OSM ) {
            if ( ! wp_style_is( 'leaflet-css' ) ) {
                // Enqueue the registered styles
                wp_enqueue_style('leaflet-css');
                wp_enqueue_style('leaflet-markercluster-css');
                wp_enqueue_style('leaflet-markercluster-default-css');
            }
	...
	private static function get_coordinates_new( $address, $skip_cache=false ) {
        ...
        } elseif ( self::API_OSM === $api_used ) {
            $coordinates = self::get_coordinates_osm( $address );
		} else {
			return sprintf(
	 ...
	 * @param float $lon
	 */
	private static function set_cached_coordinate( $address_passed, $lat, $lon ) {
		if ( self::$coordinates_cache === null ) {
			self::$coordinates_cache = self::load_cached_coordinates();
		}
	...
	public static function update_cached_coordinates( $address_passed, $address, $lat, $lon ) {
		$coordinates_set = self::get_cached_coordinates();
		if ( array_key_exists( $address_passed, $coordinates_set ) ) {
			return;
		}

		global $wpdb;

		self::set_cached_coordinate( $address_passed, $lat, $lon );

		$table_name = $wpdb->prefix . CreateDatabaseTable::TABLE_NAME;
		// Using IGNORE, because one can have the same address in multiple post fields, and no need (or way) to have
		// multiple copies of address in the cache table. So just ignore duplicate entries in cache table.
		$sql = "INSERT IGNORE INTO $table_name ( `address_passed`, `address`, `point` ) "
    ...
    /**
     * Builds the Nominatim User-Agent string.
     */
    public static function get_nominatim_user_agent() {
        // Include plugin version, domain, etc. as you prefer
        $domain = wp_parse_url( home_url(), PHP_URL_HOST );
        return sprintf( 'Toolset-Maps/%s (%s)', TOOLSET_ADDON_MAPS_VERSION, $domain );
    }

    /**
     * Resolve address to coordinates using Open Street Maps Nomatim's API
     *
     * @param string $address
     *
     * @return array|mixed|object|string|void
     */
    protected static function get_coordinates_osm( $address ) {
        // Throttle check
        if ( self::$api_call_throttle >= self::API_CALL_THROTTLE_MAX ) {
            return self::get_api_throttling_reached_message();
        } else {
        ...
        // Build the Nominatim request.
        // Example: https://nominatim.openstreetmap.org/search?format=json&q=New+York
        $args = array(
            'q' => $address,
            'format' => 'json',
            'limit' => '1',
        ...
        // Using GET
        $url = add_query_arg( $args, self::API_OSM_URL );

        // Enforce “User-Agent” header - Nominatim’s usage policy requires it.
        $request_args = array(
            'headers' => array(
                'User-Agent' => self::get_nominatim_user_agent(),
            ),
        );

        $response = wp_remote_get( $url, $request_args );
        if ( is_wp_error( $response ) ) {
            return __( 'Could not communicate with Nominatim OSM API.', 'toolset-maps' );
        }

        $json_body = wp_remote_retrieve_body( $response );
        if ( empty( $json_body ) ) {
            return __( 'No data returned from Nominatim OSM API.', 'toolset-maps' );
        }

        $data_array = json_decode( $json_body, true );
        if ( ! is_array( $data_array ) || empty( $data_array ) ) {
            return __( 'No results from Nominatim OSM API for this address.', 'toolset-maps' );
        }

        // The first result in array
        $result = $data_array[0];
        // Check required keys
        if ( ! isset( $result['lat'] ) || ! isset( $result['lon'] ) ) {
            return __( 'Malformed response from Nominatim OSM API.', 'toolset-maps' );
        }

        // Return the recognized lat/lon and a “clean” address from display_name
        return array(
            'lat'     => (float) $result['lat'],
            'lon'     => (float) $result['lon'],
            'address' => isset($result['display_name']) ? $result['display_name'] : $address,
        );
    }
	...
	protected static function get_coordinates_google( $address, $address_hash=null, $coordinates_set=null ) {
		...
		if ( is_wp_error( $response ) ) {
			// Couldn't communicate with API - schedule a cron to try later.
	...
	static function render_map( $map_id, array $map_data, $content = '' ) {
		...
		$return .= ' style="' . ( ! empty( $map_data['align'] ) ? '' : 'width:' . $map_data['map_width'] . '; ' ) . 'height:' . $map_data['map_height'] . ';"';
		$return .= ' class="wpv-addon-maps-render js-wpv-addon-maps-render js-wpv-addon-maps-render-' . $map_id . ' ' . ( ! empty( $map_data['align'] ) ? 'align' . $map_data['align'] : '' ) . '"';
        ...
        $return .= ' data-osmlayer="' . esc_attr( $map_data['osm_layer'] ) . '"';
		...
		$return .= self::maybe_preload_hidden_icon_images_html_for_azure(
			self::get_marker_icons()
		);
	...
	/**
	 * For Azure API, we need to preload icon images to be loaded by browser
	 *
	 * @param array $icons_array
	 *
	 * @return string
	 */
	public static function maybe_preload_hidden_icon_images_html_for_azure( $icons_array ) {
		$html = '';
	...
	/**
	 * @param string|bool $given_map_style
	 * @return string
	 */
	protected static function get_given_or_default_style_json( $given_map_style = false ) {
	...
	protected static function get_uploaded_json_styles() {
		return array_map(
			'Toolset_Addon_Maps_Common::make_relative_upload_link_full',
	...
	static function render_marker( $map_id, array $marker_data, $content = '' ) {
		...
		$return .= ' data-fromview="' . esc_attr( $marker_data['from_view'] ) . '"';
		$return .= '>' . $content . '</div>';
