 ...
 * License:             GPL v3
 *
 * Text Domain:         google-analytics-premium
...
final class MonsterInsights {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'google-analytics-premium' ), '6.0.0' );
	}
	...
	public function monsterinsights_lite_notice() {
		...
		?>
		<div class="error">
			<p><?php echo sprintf(esc_html__('Please %1$suninstall%2$s the MonsterInsights Lite Plugin. Your premium version of MonsterInsights may not work as expected until the Lite version is uninstalled.', 'google-analytics-premium'), '<a href="' . $url . '">', '</a>'); // phpcs:ignore ?></p>
		</div>
		<?php
	}
 ...
 * @since 8.18
 */
function monsterinsights_deactivation_hook() {
	wp_clear_scheduled_hook( 'monsterinsights_usage_tracking_cron' );
	wp_clear_scheduled_hook( 'monsterinsights_email_summaries_cron' );
	wp_clear_scheduled_hook( 'monsterinsights_ai_insights_checkin' );
...
register_deactivation_hook( __FILE__, 'monsterinsights_deactivation_hook' );
