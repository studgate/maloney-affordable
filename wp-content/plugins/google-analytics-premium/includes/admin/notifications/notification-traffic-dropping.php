...
final class MonsterInsights_Notification_Traffic_Dropping extends MonsterInsights_Notification_Event {
	...
	public function prepare_notification_data( $notification ) {
		...
		if ( ! empty( $data ) && $data['prev_sessions_difference'] < 0 ) {
			...
			$notification['title'] = __( 'Your Website Traffic is Dropping', 'google-analytics-premium' );
			// Translators: Traffic dropping notification content
			$notification['content'] = sprintf( __( 'Your website traffic is decreasing and that’s a reason to take action now. Less traffic means less opportunities to make your brand known, make relationships and ultimately sell your service or product. <br><br>Follow the marketing hacks of %1$sthis article%2$s to start growing your traffic again.', 'google-analytics-premium' ), '<a href="' . $this->build_external_link( 'https://www.monsterinsights.com/marketing-hacks-guaranteed-to-grow-your-traffic/' ) . '" target="_blank">', '</a>' );
