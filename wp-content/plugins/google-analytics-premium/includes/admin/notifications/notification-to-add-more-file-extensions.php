...
final class MonsterInsights_Notification_To_Add_More_File_Extensions extends MonsterInsights_Notification_Event {
	...
	public function prepare_notification_data( $notification ) {
		...
		if ( empty( $download_extensions ) || "doc,pdf,ppt,zip,xls,docx,pptx,xlsx" === $download_extensions ) {
			...
			$notification['title'] = __( 'Track Your Website Downloads', 'google-analytics-premium' );

			$notification['content'] = sprintf(
				/* translators: Placeholders add links to settings pages. */
				__( 'By default, MonsterInsights automatically tracks downloads of the following file extensions: doc, pdf, ppt, zip, xls, docx, pptx, and xlsx. You can easily add or remove extensions from that list in the %1$sEngagement settings%2$s of MonsterInsights.<br><br> You can view your Top Downloads report directly in the MonsterInsights %3$sPublishers report%4$s.', 'google-analytics-premium' ),
				'<a href="' . $settings_url . '">',
				'</a>',
			...
			$notification['btns']    = array(
				"add_more_file_extensions" => array(
					'url'  => $settings_url,
					'text' => __( 'Add File Extensions', 'google-analytics-premium' )
				),
			);
