...
class MonsterInsights_EEA_Compliance {
	...
	public function add_site_health_tests( $tests ) {
		...
		if ( isset( $compliant_info['compliant'] ) && ! $compliant_info['compliant'] ) {
			$tests['direct']['monsterinsights_eea_compliance_checker'] = array(
				'label' => __( 'MonsterInsights EEA Compliance Check', 'google-analytics-premium' ),
	...
	public function test_eea_compliance_checker() {
		...
		return array(
			'label'       => __( 'Check EEA Compliance with MonsterInsights', 'google-analytics-premium' ),
			'status'      => 'recommended',
			'badge'       => array(
			...
			'actions'     => sprintf(
				...
				$link,
				__( 'Check Now', 'google-analytics-premium' )
			),
