...
function monsterinsights_modify_wordpress_autoupdater_setting( $html, $plugin_file, $plugin_data ) {
	...
	$is_main_free   = isset( $plugin_data['slug'] ) && 'google-analytics-premium' === $plugin_data['slug'];
	...
	if ( $is_addon && ! $is_pro ) {
		$html = sprintf(
			...
			__( 'Enable the MonsterInsights PRO plugin to manage auto-updates', 'google-analytics-premium' )
		);
		...
		$text = __( 'Manage auto-updates', 'google-analytics-premium' );
...
function monsterinsights_automatic_updates( $update, $item ) {
	...
	$is_free = isset( $item['slug'] ) && 'google-analytics-premium' === $item['slug'];
