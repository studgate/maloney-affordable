...
function monsterinsights_prevent_version_number_removal( $src ) {
	...
	if ( ( false !== strpos( $src, 'monsterinsights' ) || false !== strpos( $src, 'google-analytics-premium' ) || false !== strpos( $src, 'google-analytics-premium' ) ) && false === strpos( $src, '?ver' ) ) {
...
function monsterinsights_maybe_add_wp_php_version_notification() {
	...
	if ( $needs_php_warning ) {
		...
		$notification['title'] = __( 'ACTION REQUIRED: Your PHP version is putting your site at risk!', 'google-analytics-premium' );
		if ( $needs_wp_warning ) {
			$notification['title'] = __( 'ACTION REQUIRED: Speed your website up 400% with a single email!', 'google-analytics-premium' );
		}
	...
	if ( $needs_wp_warning ) {
		...
		$notification['title'] = __( 'ACTION REQUIRED: Your WordPress version is putting your site at risk!', 'google-analytics-premium' );
		$notification['type']  = array( 'basic', 'lite', 'master', 'plus', 'pro' );
		// Translators: Placeholder is for the current WordPress version.
		$notification['content'] = sprintf( esc_html__( 'In the next major release of MonsterInsights we are planning to remove support for the version of WordPress you are using (version %s). This version is several years out of date, and most plugins do not support this version anymore, so you could be missing out on critical updates for performance and security already!', 'google-analytics-premium' ), $wp_version ) . "\n\n";

		$notification['content'] .= esc_html__( 'The good news: updating WordPress has never been easier and only takes a few moments.', 'google-analytics-premium' );
		// Translators: Placeholders add links to articles.
		$notification['content'] .= sprintf( esc_html__( 'To update, we recommend following this %1$sstep by step guide for updating WordPress%2$s from IsItWP and afterwards check out %3$sWhy You Should Always Use the Latest Version of WordPress%4$s on WPBeginner.', 'google-analytics-premium' ), '<a target="_blank" href="' . $isitwp_url . '">', '</a>', '<a target="_blank" href="' . $wpbeginner_url . '">', '</a>' ) . "\n\n";
...
function monsterinsights_year_in_review_notification() {
	...
	if ( monsterinsights_date_is_between( '2024-01-01', '2024-01-14' ) ) {
		...
		$notification['title']   = esc_html__( 'View 2024 Year in Review report!', 'google-analytics-premium' );
		$notification['content'] = esc_html__( 'See how your website performed this year and find tips along the way to help grow even more in 2024!', 'google-analytics-premium' );
		$notification['btns']    = array(
			'learn_more' => array(
