...
function monsterinsights_admin_menu()
{
	...
	add_menu_page(__('MonsterInsights', 'google-analytics-premium'), __('Insights', 'google-analytics-premium') . $menu_notification_indicator, 'monsterinsights_view_dashboard', $parent_slug, 'monsterinsights_reports_page', $menu_icon_inline, '100.00013467543');

	if ( ! MonsterInsights()->setup_checklist->is_dismissed() ) {
		add_submenu_page( $hook, __( 'Setup Checklist', 'google-analytics-premium' ), __( 'Setup Checklist', 'google-analytics-premium' ) . MonsterInsights()->setup_checklist->get_menu_count(), 'monsterinsights_save_settings', 'monsterinsights_settings#/setup-checklist', 'monsterinsights_settings_page' );
	}

	if ( $hook === 'monsterinsights_reports' ) {
		add_submenu_page( $parent_slug, __( 'General Reports:', 'google-analytics-premium' ), __( 'Reports', 'google-analytics-premium' ), 'monsterinsights_view_dashboard', 'monsterinsights_reports', 'monsterinsights_reports_page' );
	}

	// then settings page
	add_submenu_page( $parent_slug, __( 'MonsterInsights', 'google-analytics-premium' ), __( 'Settings', 'google-analytics-premium' ), 'monsterinsights_save_settings', 'monsterinsights_settings', 'monsterinsights_settings_page' );

	// Add dashboard submenu.
	add_submenu_page( 'index.php', __( 'General Reports:', 'google-analytics-premium' ), __( 'Insights', 'google-analytics-premium' ), 'monsterinsights_view_dashboard', 'admin.php?page=monsterinsights_reports' );
	...
	//  Site Notes
	add_submenu_page( $parent_slug, __( 'Site Notes:', 'google-analytics-premium' ), __( 'Site Notes', 'google-analytics-premium' ), 'monsterinsights_save_settings', $submenu_base . '#/site-notes' );

	// If report disabled then remove this menu.
	if ( $hook === 'monsterinsights_reports' || $is_lite ) {
		//  AI Insights
		// translators: Icon
		add_submenu_page( $parent_slug, __( 'AI Insights:', 'google-analytics-premium' ), sprintf( __( '%s AI Insights', 'google-analytics-premium' ), monsterinsights_get_ai_menu_icon() ), 'monsterinsights_save_settings', 'admin.php?page=monsterinsights_reports#/ai-insights' );
	}
	...
	if ( $is_lite || 'plus' === $license_type ) {
		...
		add_submenu_page( $parent_slug, __( 'Conversations AI:', 'google-analytics-premium' ), sprintf( __( '%s Conversations AI', 'google-analytics-premium' ), monsterinsights_get_ai_menu_icon() ), 'monsterinsights_save_settings', 'admin.php?page=monsterinsights_reports#/ai-insights/chat' );
	}

	// Add Popular Posts menu item.
	add_submenu_page( $parent_slug, __( 'Popular Posts:', 'google-analytics-premium' ), __( 'Popular Posts', 'google-analytics-premium' ), 'monsterinsights_save_settings', $submenu_base . '#/popular-posts' );

	// If report disabled then remove this menu.
	if ( $hook === 'monsterinsights_reports' || $is_lite ) {
		// Add submenu under `Insights` main menu for user journey report.
		add_submenu_page( $parent_slug, __( 'User Journey:', 'google-analytics-premium' ), __( 'User Journey', 'google-analytics-premium' ), 'monsterinsights_view_dashboard', 'admin.php?page=monsterinsights_reports#/user-journey-report' );
	}
	...
	// then SEO
	add_submenu_page($parent_slug, __('SEO', 'google-analytics-premium'), __('SEO', 'google-analytics-premium'), 'manage_options', $seo_url);

	// Google PAX
	add_submenu_page($parent_slug, __('Google Ads', 'google-analytics-premium'), __('Google Ads', 'google-analytics-premium'), 'monsterinsights_view_dashboard', $submenu_base . '#/google-ads');

	// then tools
	add_submenu_page($parent_slug, __('Tools:', 'google-analytics-premium'), __('Tools', 'google-analytics-premium'), 'manage_options', $submenu_base . '#/tools');
	...
	if (!monsterinsights_is_network_active() || (monsterinsights_is_network_active() && empty($network_key))) {
		add_submenu_page($parent_slug, __('Addons:', 'google-analytics-premium'), '<span style="color:' . monsterinsights_menu_highlight_color() . '"> ' . __('Addons', 'google-analytics-premium') . '</span>', 'monsterinsights_save_settings', $submenu_base . '#/addons');
	}

	add_submenu_page(
		$parent_slug,
		__('UserFeedback:', 'google-analytics-premium'),
		__('UserFeedback', 'google-analytics-premium') . $new_indicator,
		'manage_options',
	...
	add_submenu_page(
		$parent_slug,
		__('Privacy Compliance:', 'google-analytics-premium'),
		sprintf(
	...
	// then About Us page.
	add_submenu_page($parent_slug, __('About Us:', 'google-analytics-premium'), __('About Us', 'google-analytics-premium'), 'manage_options', $submenu_base . '#/about');
	...
	if (!monsterinsights_is_pro_version()) {
		add_submenu_page($parent_slug, __('Upgrade to Pro:', 'google-analytics-premium'), '<span class="monsterinsights-upgrade-submenu"> ' . __('Upgrade to Pro', 'google-analytics-premium') . '</span>', 'monsterinsights_save_settings', monsterinsights_get_upgrade_link('admin-menu', 'submenu', "https://www.monsterinsights.com/lite/"));
	}
}
...
function monsterinsights_automated_menu($hook){
	...
	$conditions = [
		[
			'title' => __('Earth Day', 'google-analytics-premium'),
		...
		[
			'title' => __('Cinco De Mayo!', 'google-analytics-premium'),
		...
		[
			'title' => __('Upgrade to GA4', 'google-analytics-premium'),
		...
		[
			'title' => __('Summer Sale', 'google-analytics-premium'),
		...
		[
			'title' => __('Fortune Cookie Day', 'google-analytics-premium'),
		...
		[
			'title' => __('Halloween Sale', 'google-analytics-premium'),
...
function monsterinsights_woocommerce_menu_item()
{
	if (class_exists('WooCommerce')) {
		...
		if (class_exists('MonsterInsights_eCommerce')) {
			add_submenu_page('wc-admin&path=/analytics/overview', __('Insights', 'google-analytics-premium'), __('Insights', 'google-analytics-premium'), 'monsterinsights_view_dashboard', admin_url('admin.php?page=monsterinsights_reports#/ecommerce'), '', 2);
		} else {
			$submenu_base = add_query_arg('page', 'monsterinsights_settings', admin_url('admin.php'));
			add_submenu_page('wc-admin&path=/analytics/overview', __('Insights', 'google-analytics-premium'), __('Insights', 'google-analytics-premium'), 'manage_options', $submenu_base . '#/woocommerce-insights', '', 1);
		}
	}
}
...
function monsterinsights_network_admin_menu()
{
	...
	add_menu_page(__('Network Settings:', 'google-analytics-premium'), __('Insights', 'google-analytics-premium') . $menu_notification_indicator, 'monsterinsights_save_settings', 'monsterinsights_network', 'monsterinsights_network_page', $menu_icon_inline, '100.00013467543');

	add_submenu_page($hook, __('Network Settings:', 'google-analytics-premium'), __('Network Settings', 'google-analytics-premium'), 'monsterinsights_save_settings', 'monsterinsights_network', 'monsterinsights_network_page');

	add_submenu_page($hook, __('General Reports:', 'google-analytics-premium'), __('Reports', 'google-analytics-premium'), 'monsterinsights_view_dashboard', 'monsterinsights_reports', 'monsterinsights_reports_page');
	...
	// then seo
	add_submenu_page($hook, __('SEO:', 'google-analytics-premium'), __('SEO', 'google-analytics-premium'), 'manage_options', $seo_url, 'monsterinsights_seo_page');

	// then addons
	add_submenu_page($hook, __('Addons:', 'google-analytics-premium'), '<span style="color:' . monsterinsights_menu_highlight_color() . '"> ' . __('Addons', 'google-analytics-premium') . '</span>', 'monsterinsights_save_settings', $submenu_base . '#/addons');

	$submenu_base = add_query_arg('page', 'monsterinsights_network', network_admin_url('admin.php'));

	// Add About us page.
	add_submenu_page($hook, __('About Us:', 'google-analytics-premium'), __('About Us', 'google-analytics-premium'), 'manage_options', $submenu_base . '#/about');
}
...
{
	$docs = '<a title="' . esc_attr__('MonsterInsights Knowledge Base', 'google-analytics-premium') . '" href="' . monsterinsights_get_url('all-plugins', 'kb-link', "https://www.monsterinsights.com/docs/") . '">' . esc_html__('Documentation', 'google-analytics-premium') . '</a>';
	array_unshift($links, $docs);

	// If Lite, support goes to forum. If pro, it goes to our website
	if (monsterinsights_is_pro_version()) {
		$support = '<a title="MonsterInsights Pro Support" href="' . monsterinsights_get_url('all-plugins', 'pro-support-link', "https://www.monsterinsights.com/my-account/support/") . '">' . esc_html__('Support', 'google-analytics-premium') . '</a>';
		array_unshift($links, $support);
	} else {
		$support = '<a title="MonsterInsights Lite Support" href="' . monsterinsights_get_url('all-plugins', 'lite-support-link', "https://www.monsterinsights.com/lite-support/") . '">' . esc_html__('Support', 'google-analytics-premium') . '</a>';
		array_unshift($links, $support);
	}

	if (is_network_admin()) {
		$settings_link = '<a href="' . esc_url(network_admin_url('admin.php?page=monsterinsights_network')) . '">' . esc_html__('Network Settings', 'google-analytics-premium') . '</a>';
	} else {
		$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=monsterinsights_settings')) . '">' . esc_html__('Settings', 'google-analytics-premium') . '</a>';
	}
	...
	if (!monsterinsights_is_pro_version()) {
		$get_pro = '<a title="' . esc_attr__('Get MonsterInsights Pro', 'google-analytics-premium') . '" target="_blank" rel="noopener" href="' . monsterinsights_get_upgrade_link('all-plugins', 'upgrade-link', "https://www.monsterinsights.com/lite/") . '" style="font-weight:700; color: #1da867;">' . esc_html__('Get MonsterInsights Pro', 'google-analytics-premium') . '</a>';
...
function monsterinsights_admin_setup_notices()
{
	...
	if ( !empty($profile['ua']) && empty($profile['v4']) && !monsterinsights_is_own_admin_page() ) {
		$title = __('Urgent: Your Website is Not Tracking Any Google Analytics Data!', 'google-analytics-premium');
		$message = __('Google Analytics 3 (UA) and support was sunset on July 1, 2023. Your website is currently NOT tracking any analytics. </br>Create or connect a new Google Analytics 4 property immediately to start tracking.', 'google-analytics-premium');
		...
		echo '<a href="https://www.monsterinsights.com/docs/connect-google-analytics/"
			...
			__( 'Learn How to Create a GA4 Property', 'google-analytics-premium' ) . // phpcs:ignore
			'</a><br>';
		echo '<a href="' . esc_url($wizard_url) . '">' .
			__( 'Connect a Property', 'google-analytics-premium' ) . // phpcs:ignore
			'</a><br>';
		echo '</p>';
		echo '</div>';
	...
	if ( ! is_network_admin() && ! monsterinsights_get_v4_id() && ! defined( 'MONSTERINSIGHTS_DISABLE_TRACKING' ) && ! monsterinsights_is_own_admin_page() ) {
		...
		$title        = esc_html__( 'Please Setup Website Analytics to See Audience Insights', 'google-analytics-premium' );
		$primary      = esc_html__( 'Please Connect Your Website to MonsterInsights', 'google-analytics-premium' );
		...
		$secondary    = esc_html__( 'Learn More', 'google-analytics-premium' );
		$urltwo       = $submenu_base . '#/about/getting-started';
		$disclaimer   = __( 'Note: You will be transfered to MonsterInsights.com to complete the setup wizard.', 'google-analytics-premium' );
		$message      = esc_html__( 'MonsterInsights, the #1 WordPress Analytics Plugin, helps you easily connect your website to Google Analytics, so that you can see how people find and use your website. Over 3 million website owners use MonsterInsights to see the stats that matter and grow their business.', 'google-analytics-premium' );
	...
	if (monsterinsights_is_pro_version() && empty($key)) {
		...
		$message = sprintf(esc_html__('Warning: No valid license key has been entered for MonsterInsights. You are currently not getting updates, and are not able to view reports. %1$sPlease click here to enter your license key and begin receiving updates and reports.%2$s', 'google-analytics-premium'), '<a href="' . esc_url($page) . '">', '</a>');
	...
	if (monsterinsights_is_pro_version()) {
		...
		if (MonsterInsights()->license->get_site_license_key()) {
			if (MonsterInsights()->license->site_license_expired()) {
				...
				$message = sprintf(esc_html__('Your license key for MonsterInsights has expired. %1$sPlease click here to renew your license key.%2$s', 'google-analytics-premium'), '<a href="' . monsterinsights_get_url('admin-notices', 'expired-license', "https://www.monsterinsights.com/login/") . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>');
			} else if (MonsterInsights()->license->site_license_disabled()) {
				$message = esc_html__('Your license key for MonsterInsights has been disabled. Please use a different key.', 'google-analytics-premium');
			} else if (MonsterInsights()->license->site_license_invalid()) {
				$message = esc_html__('Your license key for MonsterInsights is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key.', 'google-analytics-premium');
			}
		} else if (MonsterInsights()->license->get_network_license_key()) {
			if (MonsterInsights()->license->network_license_expired()) {
				// Translators: Adds a link to renew license.
				$message = sprintf(esc_html__('Your network license key for MonsterInsights has expired. %1$sPlease click here to renew your license key.%2$s', 'google-analytics-premium'), '<a href="' . monsterinsights_get_url('admin-notices', 'expired-license', "https://www.monsterinsights.com/login/") . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>');
			} else if (MonsterInsights()->license->network_license_disabled()) {
				$message = esc_html__('Your network license key for MonsterInsights has been disabled. Please use a different key.', 'google-analytics-premium');
			} else if (MonsterInsights()->license->network_license_invalid()) {
				$message = esc_html__('Your network license key for MonsterInsights is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key.', 'google-analytics-premium');
			}
		}
		if (!empty($message)) {
			echo '<div class="error"><p>' . $message . '</p></div>'; // phpcs:ignore
	...
	if (current_user_can('update_core')) {
		...
		if (version_compare(phpversion(), $compatible_php_version['required'], '<')) {
			...
			$message = sprintf(esc_html__('Your site is running an outdated, insecure version of PHP (%1$s), which could be putting your site at risk for being hacked.%4$sWordPress stopped supporting your PHP version in April, 2019.%4$sUpdating PHP only takes a few minutes and will make your website significantly faster and more secure.%4$s%2$sLearn more about updating PHP%3$s', 'google-analytics-premium'), phpversion(), '<a href="' . $url . '" target="_blank">', '</a>', '<br>');
		} else if (version_compare(phpversion(), $compatible_php_version['warning'], '<')) {
			// Translators: Placeholders add the PHP version, a link to the MonsterInsights blog and a line break.
			$message = sprintf(esc_html__('Your site is running an outdated, insecure version of PHP (%1$s), which could be putting your site at risk for being hacked.%4$sWordPress stopped supporting your PHP version in November, 2019.%4$sUpdating PHP only takes a few minutes and will make your website significantly faster and more secure.%4$s%2$sLearn more about updating PHP%3$s', 'google-analytics-premium'), phpversion(), '<a href="' . $url . '" target="_blank">', '</a>', '<br>');
		} else if (version_compare(phpversion(), $compatible_php_version['recommended'], '<')) {
			// Translators: Placeholders add the PHP version, a link to the MonsterInsights blog and a line break.
			$message = sprintf(esc_html__('Your site is running an outdated, insecure version of PHP (%1$s), which could be putting your site at risk for being hacked.%4$sWordPress is working towards discontinuing support for your PHP version.%4$sUpdating PHP only takes a few minutes and will make your website significantly faster and more secure.%4$s%2$sLearn more about updating PHP%3$s', 'google-analytics-premium'), phpversion(), '<a href="' . $url . '" target="_blank">', '</a>', '<br>');
		}
		...
		/* else if ( version_compare( $wp_version, '5.0', '<' ) ) {
			...
			$message = sprintf( esc_html__( 'Your site is running an outdated version of WordPress (%1$s).%4$sMonsterInsights will stop supporting WordPress versions lower than 5.0 in 2021.%4$sUpdating WordPress takes just a few minutes and will also solve many bugs that exist in your WordPress install.%4$s%2$sLearn more about updating WordPress%3$s', 'google-analytics-premium' ), $wp_version, '<a href="' . $url . '" target="_blank">', '</a>', '<br>' );
		...
		//  $url = monsterinsights_get_url( 'global-notice', 'settings-page', 'https://www.monsterinsights.com/docs/update-wordpress/' );
		//  $message = sprintf( esc_html__( 'Your site is running an outdated version of WordPress (%1$s).%4$sMonsterInsights will stop supporting WordPress versions lower than 4.9 in October, 2019.%4$sUpdating WordPress takes just a few minutes and will also solve many bugs that exist in your WordPress install.%4$s%2$sLearn more about updating WordPress%3$s', 'google-analytics-premium' ), $wp_version, '<a href="' . $url . '" target="_blank">', '</a>', '<br>' );
		//  echo '<div class="error"><p>'. $message.'</p></div>';
		//  return;
		// }
	...
	//                 echo esc_html__( 'Allow MonsterInsights to track plugin usage? Opt-in to tracking and our newsletter to stay informed of the latest changes to MonsterInsights and help us ensure compatibility.', 'google-analytics-premium' );
	//                 echo '&nbsp;<a href="' . esc_url( $optin_url ) . '" class="button-secondary">' . __( 'Allow', 'google-analytics-premium' ) . '</a>';
	//                 echo '&nbsp;<a href="' . esc_url( $optout_url ) . '" class="button-secondary">' . __( 'Do not allow', 'google-analytics-premium' ) . '</a>';
	//                 echo '</p></div>';
	//                 return;
	//             } else {
	//                 // is testing site
	...
	$manual_text = sprintf(esc_html__('Important: You are currently using manual GA4 Measurement ID output. We highly recommend %1$sauthenticating with MonsterInsights%2$s so that you can access our new reporting area and take advantage of new MonsterInsights features.', 'google-analytics-premium'), '<a href="' . $url . '">', '</a>');
	...
	if ($migrated > 0) {
		...
		// Translators: Placeholders add links to the settings panel.
		$text        = esc_html__('Click %1$shere%2$s to reauthenticate to be able to access reports. For more information why this is required, see our %3$sblog post%4$s.', 'google-analytics-premium');
	...
	//             echo '<p>';
	//             echo sprintf( esc_html__( 'Important: Please %1$sconfigure the Automatic Updates Settings%2$s in MonsterInsights.', 'google-analytics-premium' ), '<a href="' . $url .'">', '</a>' );
	//             echo '</p>';
	//         echo '</div>';
	//         return;
	//     }
	...
	if (!monsterinsights_is_pro_version() && class_exists('WooCommerce') && $is_plugins_page) {
		if (!isset($notices['monsterinsights_woocommerce_tracking_available'])) {
			...
			$woo_notice_button = sprintf(
				...
				esc_html__('%1$sGet MonsterInsights Pro%2$s', 'google-analytics-premium'),
			...
			$woo_notice_offer = sprintf(
				...
				__('Save <span>50%</span> Off MonsterInsights Pro', 'google-analytics-premium')
			);
			...
			echo sprintf(
				$woo_notice_template,
				$woo_notice_style,
				__('Add eCommerce Analytics to your WooCommerce Store', 'google-analytics-premium'),
				__('Unlock all of our advanced eCommerce features specifically designed to help your store make more money..', 'google-analytics-premium'),
				__('MonsterInsights Pro users instantly gain access to valuable insights such as average order value, conversion rates, as well as marketing performance with UTM tracking.', 'google-analytics-premium'),
				__('And by upgrading, Pro users also get enhanced tracking for Forms, User Journeys, PPC Pixels, Custom UserID tracking, SEO Reports, and much more.', 'google-analytics-premium'),
				__('Start making better data-driven decisions today!', 'google-analytics-premium'),
				$woo_notice_offer,
				$woo_notice_button
			);
			return;
	...
	if (!monsterinsights_is_pro_version() && class_exists('Easy_Digital_Downloads') && $is_plugins_page) {
		if (!isset($notices['monsterinsights_edd_tracking_available'])) {
			...
			echo '<p><strong>';
			echo esc_html('Enhanced Ecommerce Analytics for Your Easy Digital Downloads Store', 'google-analytics-premium');
			echo '</strong></p>';
			...
			echo '<p>';
			echo esc_html('MonsterInsights Pro gives you detailed stats and insights about your customers.', 'google-analytics-premium');
			echo '</p>';
			echo '<p>';
			echo esc_html('This helps you make data-driven decisions about your content, and marketing strategy so you can increase your website traffic, leads, and sales.', 'google-analytics-premium');
			echo '</p>';
			echo '<p>';
			echo esc_html('Pro customers also get Form Tracking, Custom Dimensions Tracking, UserID Tracking and much more.', 'google-analytics-premium');
			echo '</p>';
			echo '<p>';
			echo esc_html('Start making data-driven decisions to grow your business.', 'google-analytics-premium');
			echo '</p>';
			echo sprintf(esc_html__('%1$sGet MonsterInsights Pro%2$s', 'google-analytics-premium'), '<a class="button button-primary button-hero" target="_blank" href="' . esc_url(monsterinsights_get_upgrade_link('admin-notices', 'edd-upgrade')) . '">', ' &raquo;</a>');
			echo '</p>';
	...
	if (isset($notices['monsterinsights_cross_domains_extracted']) && false === $notices['monsterinsights_cross_domains_extracted']) {
		...
		$message = sprintf(esc_html__('Warning: MonsterInsights found cross-domain settings in the custom code field and converted them to the new settings structure.  %1$sPlease click here to review and remove the code no longer needed.%2$s', 'google-analytics-premium'), '<a href="' . esc_url($page) . '">', '</a>');
...
function monsterinsights_ads_addon_installed_notice() {
	...
	if ( monsterinsights_is_pro_version() && MonsterInsights()->license->get_license_type() === 'pro' ) {
		...
		$button_text = esc_html__( 'Install Now', 'google-analytics-premium' );
		$button_target = '_self';
	} else {
		$addons_url = monsterinsights_get_upgrade_link('admin-notices', 'ads-addon-activated', 'https://www.monsterinsights.com/lite/');
		$button_text = esc_html__( 'Upgrade Now', 'google-analytics-premium' );
...
function monsterinsights_in_admin_footer() {
	...
	$links = [
		[
			'text' => __( 'Support', 'google-analytics-premium' ),
			'link' => $is_pro ? monsterinsights_get_url('footer_link', 'made-with-love', 'https://www.monsterinsights.com/support/') : 'https://wordpress.org/support/plugin/google-analytics-premium/',
			'target' => '_blank',
		],
		[
			'text' => __( 'Docs', 'google-analytics-premium' ),
			'link' => monsterinsights_get_url('footer_link', 'made-with-love', 'https://www.monsterinsights.com/docs/'),
			'target' => '_blank',
		],
		[
			'text' => __( 'Free Plugins', 'google-analytics-premium' ),
	...
	echo sprintf(esc_html__('Made with %1$s by the MonsterInsights Team', 'google-analytics-premium'), '<span class="monsterinsights-footer-love-icon">♥</span>');
	$links_output = [];
...
function monsterinsights_wpconsent_install_notice() {
	...
	<div class="monsterinsights-wpconsent-notice-box notice" id="monsterinsights-wpconsent-notice">
		...
		<div class="monsterinsights-wpconsent-content">
			...
			<h3><?php esc_html_e('Make Your Website Analytics Compliant with Privacy Laws', 'google-analytics-premium'); ?></h3>
			<p><?php esc_html_e('Privacy laws like GDPR, CCPA, and others require user consent before you can track visitors. Our sister plugin, WPConsent, helps you add a smart cookie consent banner for better privacy compliance with Google Analytics.', 'google-analytics-premium'); ?></p>
			<div class="monsterinsights-wpconsent-buttons">
				<a class="monsterinsights-wpconsent-btn-primary" href="<?php echo esc_url( admin_url() ) ?>admin.php?page=monsterinsights_settings#/wpconsent?installnow=1">
					<?php
						if ( $plugin_installed ) {
							esc_html_e('Activate', 'google-analytics-premium');
						} else {
							esc_html_e('Install Now', 'google-analytics-premium');
						}
					 ?>
				</a>
				<a class="monsterinsights-wpconsent-btn-secondary" href="<?php echo esc_url( admin_url() ) ?>admin.php?page=monsterinsights_settings#/wpconsent"><?php esc_html_e('Learn More', 'google-analytics-premium'); ?></a>
			</div>
		</div>
	</div>
	<?php
