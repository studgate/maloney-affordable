...
class MonsterInsights_AM_Deactivation_Survey {
				...
				$(function() {
					...
					$form.submit(function(event) {
						...
						if (! $form.find('input[type=radio]:checked').val()) {
							$form.find('.am-deactivate-survey-footer').prepend('<span class="error"><?php echo esc_js( __( 'Please select an option', 'google-analytics-premium' ) ); ?></span>');
							return;
						}
	...
	public function modal() {
		...
		$options = array(
			1 => array(
				'title' => esc_html__( 'I no longer need the plugin', 'google-analytics-premium' ),
			),
			2 => array(
				'title'   => esc_html__( 'I\'m switching to a different plugin', 'google-analytics-premium' ),
				'details' => esc_html__( 'Please share which plugin', 'google-analytics-premium' ),
			),
			3 => array(
				'title' => esc_html__( 'I couldn\'t get the plugin to work', 'google-analytics-premium' ),
			),
			4 => array(
				'title' => esc_html__( 'It\'s a temporary deactivation', 'google-analytics-premium' ),
			),
			5 => array(
				'title'   => esc_html__( 'Other', 'google-analytics-premium' ),
				'details' => esc_html__( 'Please share the reason', 'google-analytics-premium' ),
			),
		);
		?>
		<div class="am-deactivate-survey-modal" id="am-deactivate-survey-<?php echo esc_attr($this->plugin); ?>">
			<div class="am-deactivate-survey-wrap">
				<form class="am-deactivate-survey" method="post">
					<button type="button" class="am-deactivate-survey-close" aria-label="<?php echo esc_attr__( 'Close', 'google-analytics-premium' ); ?>">×</button>
					<span class="am-deactivate-survey-title"><span class="dashicons dashicons-testimonial"></span><?php echo ' ' . esc_html__( 'Quick Feedback', 'google-analytics-premium' ); ?></span>
					<span class="am-deactivate-survey-desc"><?php printf( esc_html__( 'If you have a moment, please share why you are deactivating %s:', 'google-analytics-premium' ), $this->name ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- false positive  ?></span>
					...
					<div class="am-deactivate-survey-footer">
						<button type="submit" class="am-deactivate-survey-submit button button-primary button-large"><?php printf( esc_html__( 'Submit %s Deactivate', 'google-analytics-premium' ), '&amp;' ); ?></button>
						<a href="#" class="am-deactivate-survey-deactivate"><?php printf( esc_html__( 'Skip %s Deactivate', 'google-analytics-premium' ), '&amp;' ); ?></a>
					</div>
				</form>
			</div>
		</div>
