...
class MonsterInsights_SiteNotes_Controller {
	...
	public function get_notes() {
		...
		if ( ! current_user_can( 'monsterinsights_view_dashboard' ) && ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes.", 'google-analytics-premium' ),
				)
			);
		}
	...
	public function get_categories() {
		...
		if ( ! current_user_can( 'monsterinsights_view_dashboard' ) && ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes categories.", 'google-analytics-premium' ),
				)
			);
		}
	...
	public function save_note() {
		...
		if ( ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update notes.", 'google-analytics-premium' ),
				)
			);
		}
	...
	public function save_category() {
		...
		if ( ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update categories.", 'google-analytics-premium' ),
				)
			);
		}
		...
		if (empty($category->name)) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __('Please add a category name', 'google-analytics-premium'),
				)
			);
		}
		...
		if (200 < mb_strlen($category->name)) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __('You can\'t exceed the 200 characters length for each site note.', 'google-analytics-premium'),
				)
			);
		}
	...
	public function trash_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note to trash!', 'google-analytics-premium'),
				)
			);
		}
	...
	public function restore_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note(s) to restore!', 'google-analytics-premium'),
				)
			);
		}
	...
	public function delete_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note(s) to delete!', 'google-analytics-premium'),
				)
			);
		}
	...
	public function delete_categories() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a category to delete!', 'google-analytics-premium'),
				)
			);
		}
	...
	public function export_notes() {
		...
		$headers = array(
			__('Date', 'google-analytics-premium'),
			__('Site Note', 'google-analytics-premium'),
			__('Category', 'google-analytics-premium'),
		...
		foreach ($items['items'] as $item) {
			$item_media = __('NA', 'google-analytics-premium');
	...
	public function export_notes_to_ga4() {
		...
		if ( empty( $annotations ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No annotations data provided.', 'google-analytics-premium' ),
				)
			);
		}
		...
		if (
			...
			wp_send_json_error(
				array(
					'message' => __( 'You must be properly authenticated with MonsterInsights to export annotations.', 'google-analytics-premium' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => __( 'Annotations exported successfully.', 'google-analytics-premium' ),
				'data'    => $response,
			)
		);
	}
	...
	public function deleted_note_from_ga4_single($ga4_annotation_id) {
		...
		if (!(MonsterInsights()->auth->is_authed() || MonsterInsights()->auth->is_network_authed())) {
			return new WP_Error('not_authenticated', __('You must be properly authenticated with MonsterInsights to delete annotations.', 'google-analytics-premium'));
		}
	...
	public function import_notes_from_ga4() {
		...
		if ( $skipped_count > 0 ) {
			$message .= ' ' . sprintf(
				__( '%d annotations were skipped (already exist).', 'google-analytics-premium' ),
				$skipped_count
			);
		}
		...
		<div class="monsterinsights-metabox" id="monsterinsights-metabox-site-notes">
			<div class="monsterinsights-metabox-input monsterinsights-metabox-input-checkbox">
				<label class="">
					...
					<span class="monsterinsights-metabox-input-checkbox-label"><?php _e('Add a Site Note', 'google-analytics-premium'); ?></span>
				</label>
			</div>
			...
			<div id="site-notes-active-container" class="<?php echo (!$sitenote_active ? 'hidden' : ''); ?>">
				...
				<div class="monsterinsights-metabox-input monsterinsights-metabox-select">
					<label>
						<?php _e('Category', 'google-analytics-premium'); ?>
