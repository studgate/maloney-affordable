	...
	class MonsterInsights_Compatibility_Check {
		...
		public function maybe_deactivate_plugin( $plugin ) {
			...
			if ( ! empty( $compatible_php_version['required'] ) && ! $this->is_php_compatible() ) {
				...
				wp_die(
					sprintf( esc_html__( 'Sorry, but your current version of PHP does not meet the minimum required version %1$s%2$s%3$s or newer to run MonsterInsights properly. For information on how to upgrade your PHP version, contact your web host. %4$sClick here to return to the Dashboard%5$s.', 'google-analytics-premium' ),
						'<strong>',
			...
			if ( ! empty( $compatible_wp_version['required'] ) && ! $this->is_wp_compatible() ) {
				...
				wp_die(
					sprintf(
						esc_html__( 'Sorry, but your WordPress version is not %1$s%2$s%3$s or newer. Please update your WordPress version and then activate MonsterInsights. For help on how to update your WordPress %4$sclick here%5$s.', 'google-analytics-premium' ),
						'<strong>',
		...
		public function display_php_notice() {
			...
			<div class="error">
				<p>
					...
					// Translators: HTML strong tag starts, PHP version number, HTML string tag end, plugins page Link tag starts with url and plugins page link tag ends.
					echo sprintf(
						esc_html__( 'Sorry, but your current version of PHP does not meet the minimum required version %1$s%2$s%3$s or newer to run MonsterInsights properly. For information on how to upgrade your PHP version, contact your web host. %4$sClick here to return to the Dashboard%5$s.', 'google-analytics-premium' ),
						'<strong>',
						$compatible_php_version['required'], // phpcs:ignore
						'</strong>',
