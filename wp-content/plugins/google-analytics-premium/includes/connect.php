...
class MonsterInsights_Connect {
	...
	public function generate_connect_url() {
		...
		if ( ! monsterinsights_can_install_plugins() ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Oops! You are not allowed to install plugins. Please contact your site administrator.', 'google-analytics-premium' ) ) );
		}
		...
		if ( empty( $key ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Please enter your license key to connect.', 'google-analytics-premium' ),
				)
			);
		}
		...
		if ( ! is_wp_error( $active ) ) {
			...
			wp_send_json_error( array(
				'message' => esc_html__( 'You already have MonsterInsights Pro installed.', 'google-analytics-premium' ),
				'reload'  => true,
			) );
		}
	...
	public function process() {
		...
		$error = sprintf(
			esc_html__( 'Oops! We could not automatically install an upgrade. Please install manually by visiting %1$smonsterinsights.com%2$s.', 'google-analytics-premium' ),
		...
		if ( monsterinsights_is_pro_version() ) {
			wp_send_json_success( esc_html__( 'Plugin installed & activated.', 'google-analytics-premium' ) );
		}
		...
		if ( empty( $license['key'] ) ) {
			wp_send_json_error( new WP_Error( '403', esc_html__( 'You are not licensed.', 'google-analytics-premium' ) ) );
		}
		...
		if ( $installer->plugin_info() ) {
			...
			if ( ! is_wp_error( $activated ) ) {
				...
				wp_send_json_error( esc_html__( 'Please activate MonsterInsights Pro from your WordPress plugins page.', 'google-analytics-premium' ) );
			}
		}
		wp_send_json_error( $error );
	}
