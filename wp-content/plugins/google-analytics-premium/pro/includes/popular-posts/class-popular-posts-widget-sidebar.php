...
class MonsterInsights_Popular_Posts_Widget_Sidebar extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html_x( 'Display popular posts.', 'Widget', 'google-analytics-premium' ),
		);
		...
		// Load widget.
		parent::__construct( $widget_slug, esc_html_x( 'Popular Posts - MonsterInsights', 'Widget', 'google-analytics-premium' ), $widget_ops, $control_ops );
	}
	...
	public function form( $instance ) {
		...
		$this->text_input( 'title', _x( 'Widget Title:', 'Widget', 'google-analytics-premium' ), $instance );
		?>
		<p>
			<input type="checkbox"
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'display_title' ) ); ?>">
				<?php echo esc_html( _x( 'Display Widget Title', 'Widget', 'google-analytics-premium' ) ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'post_count' ) ); ?>">
				<?php echo esc_html( _x( 'Number of posts to display:', 'Widget', 'google-analytics-premium' ) ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'theme' ) ); ?>">
				<?php echo esc_html( _x( 'Theme:', 'Widget', 'google-analytics-premium' ) ); ?>
			</label>
		...
		<div class="monsterinsights-widget-theme-preview">
			<span class="monsterinsights-widget-theme-preview-label">
				<?php esc_html_e( 'Theme Preview', 'google-analytics-premium' ); ?>
			</span>
			<div
		...
		<?php if ( ! empty( $theme['styles']['title']['color'] ) ) {
			$this->color_input( 'title_color', _x( 'Title Color:', 'Widget', 'google-analytics-premium' ), $instance );
			?>
			<?php
		}
		if ( ! empty( $theme['styles']['title']['size'] ) ) {
			$this->size_input( 'title_size', _x( 'Title Font Size:', 'Widget', 'google-analytics-premium' ), $instance, $title_font_sizes );
		}
		if ( ! empty( $theme['styles']['label']['color'] ) ) {
			$this->color_input( 'label_color', _x( 'Label Color:', 'Widget', 'google-analytics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['label']['editable'] ) && ! empty( $theme['styles']['label']['text'] ) ) {
			$this->text_input( 'label_text', _x( 'Label Text:', 'Widget', 'google-analytics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['meta']['color'] ) ) {
			$this->color_input( 'meta_color', _x( 'Meta Color:', 'Widget', 'google-analytics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['meta']['size'] ) ) {
			$this->size_input( 'meta_size', _x( 'Meta Font Size:', 'Widget', 'google-analytics-premium' ), $instance, $meta_font_sizes );
		}
		if ( ! empty( $theme['styles']['background']['border'] ) ) {
			$this->color_input( 'background_border', _x( 'Border Color:', 'Widget', 'google-analytics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['background']['color'] ) ) {
			$this->color_input( 'background_color', _x( 'Background Color:', 'Widget', 'google-analytics-premium' ), $instance );
		}
		if ( ! empty( $theme['styles']['comments']['color'] ) ) {
			$this->color_input( 'comments_color', _x( 'Comments Count Color:', 'Widget', 'google-analytics-premium' ), $instance );
		}
		?>
		<p>

			<label class="monsterinsights-label-block">
				<?php echo esc_html( _x( 'Only Show Posts from These Categories:', 'Widget', 'google-analytics-premium' ) ); ?>
			</label>
		...
		<p>
			<?php if ( ! empty( $theme['styles']['meta']['author'] ) ) { ?>
				<label class="monsterinsights-label-block">
					...
					<?php esc_html_e( 'Display Author', 'google-analytics-premium' ); ?>
				</label>
			<?php } ?>
			<?php if ( ! empty( $theme['styles']['meta']['date'] ) ) { ?>
				<label class="monsterinsights-label-block">
					...
					<?php esc_html_e( 'Display Date', 'google-analytics-premium' ); ?>
				</label>
			<?php } ?>
			<?php if ( ! empty( $theme['styles']['meta']['comments'] ) ) { ?>
				<label class="monsterinsights-label-block">
					...
					<?php esc_html_e( 'Display Comments', 'google-analytics-premium' ); ?>
				</label>
			<?php } ?>
		</p>
		<?php
		...
		<p>
			...
			<select id="<?php echo esc_attr( $this->get_field_id( $name ) ); ?>"
				...
				<?php foreach ( $range as $font_size ) { ?>
					<option
						value="<?php echo absint( $font_size ); ?>" <?php selected( $instance[ $name ], $font_size ); ?>><?php printf( esc_html_x( '%dpx', 'google-analytics-premium' ), $font_size ); // phpcs:ignore ?></option>
				<?php } ?>
			</select>
		</p>
		<?php
