...
class MonsterInsights_Popular_Posts_Widget extends MonsterInsights_Popular_Posts {
	...
	public function get_rendered_html( $atts ) {
		...
		foreach ( $posts as $post ) {
			...
			$html .= '<div class="monsterinsights-widget-popular-posts-meta" ' . esc_attr($this->get_element_style( $theme, 'meta', $atts )) . '>';
			if ( $show_author ) {
				// Translators: Placeholder for author name.
				$html .= '<span class="monsterinsights-widget-popular-posts-author">' . sprintf( __( 'by %s', 'google-analytics-premium' ), esc_html($post['author_name']) ) . '</span>';
			}
			...
			if ( $show_date ) {
				$html .= '<span class="monsterinsights-widget-popular-posts-date">' . esc_html( $post['date'] ) . '</span>';
			}
			$html .= '</div>';
			if ( $show_comments ) {
				$html .= '<span class="monsterinsights-widget-popular-posts-comments" ' . esc_attr($this->get_element_style( $theme, 'comments', $atts )) . '><svg width="13" height="12" viewBox="0 0 13 12" fill="#393F4C" xmlns="http://www.w3.org/2000/svg" '
				. 'style="'
				. esc_attr(
					str_replace(
						'color',
	...
	public function build_inline_styles() {
		...
		foreach ( $themes as $theme_key => $theme_styles ) {
			...
			if ( ! empty( $theme_styles['meta'] ) ) {
				$styles .= '.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts.monsterinsights-widget-popular-posts-' . esc_attr($theme_key) . ' .monsterinsights-widget-popular-posts-list .monsterinsights-widget-popular-posts-meta span {';
			...
			if ( ! empty( $theme_styles['comments'] ) ) {
				$styles .= '.monsterinsights-widget-popular-posts.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts-' . esc_attr($theme_key) . ' .monsterinsights-widget-popular-posts-list .monsterinsights-widget-popular-posts-comments {';
				...
				$styles .= '}';
				$styles .= '.monsterinsights-widget-popular-posts.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts-' . esc_attr($theme_key) . ' .monsterinsights-widget-popular-posts-list .monsterinsights-widget-popular-posts-comments svg {';
			...
			if ( ! empty( $theme_styles['border']['color2'] ) ) {
				$styles .= '.monsterinsights-popular-posts-styled.monsterinsights-widget-popular-posts-' . esc_attr($theme_key) . ' .monsterinsights-inline-popular-posts-border-2 {';
