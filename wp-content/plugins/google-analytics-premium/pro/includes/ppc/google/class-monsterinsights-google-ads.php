...
class MonsterInsights_Google_Ads
{
	...
	public function get_access_token()
	{
		...
		if (! monsterinsights_is_authed()) {
			return new WP_Error(
				'not_authed',
				sprintf(
					__('To use this feature, please connect to MonsterInsights. <a href="%s">Click here to connect.</a>', 'google-analytics-premium'),
	...
	public function reset_experience() {
		...
		wp_send_json_success(array(
			'message' => __('Google Ads experience reset successfully.', 'google-analytics-premium'),
		));
	}
	...
	public function get_conversion_tracking_id()
	{
		...
		return new WP_Error('conversion_tracking_id_not_found', __('Conversion tracking id not found.', 'google-analytics-premium'));
	}
	...
	public function update_ads_setting()
	{
		...
		if (! current_user_can('monsterinsights_save_settings')) {
			wp_send_json_error(array(
				'message' => __('You do not have permission to update this setting.', 'google-analytics-premium'),
			));
		}

		if (! isset($_POST['key']) || ! isset($_POST['value'])) {
			wp_send_json_error(array(
				'message' => __('Invalid request.', 'google-analytics-premium'),
			));
		}
	...
	public function get_ads_settings() {
		...
		if (! current_user_can('monsterinsights_save_settings')) {
			wp_send_json_error(array(
				'message' => __('You do not have permission to view these settings.', 'google-analytics-premium'),
			));
		}
