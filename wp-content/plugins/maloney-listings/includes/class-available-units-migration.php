<?php
/**
 * Available Units Migration
 * Migrates available units data from Ninja Table 790 to listing custom fields
 * 
...
class Maloney_Listings_Available_Units_Migration {
    
    private $ninja_table_id = 790;
    ...
    private function normalize_unit_type($bedrooms) {
        ...
        // Return original if no match (e.g., "Single Room Occupancy (SRO)")
        return $bedrooms;
    }
    ...
    private function ensure_unit_size_option_exists($unit_type) {
        ...
        $field_slug = 'availability-bedrooms';
    ...
    private function normalize_property_name($name) {
        ...
        // Common variations
        $variations = array(
            'orchards' => 'orchard apartments',
            'orchard' => 'orchard apartments',
        );
    ...
    /**
     * Update listing with available units data
     * Uses repetitive fields structure - one entry per row
     */
    private function update_listing_available_units($listing_id, $rows_data) {
        // Clear existing repetitive field values
        delete_post_meta($listing_id, 'wpcf-availability-property');
        delete_post_meta($listing_id, 'wpcf-availability-town');
        delete_post_meta($listing_id, 'wpcf-availability-bedrooms');
        delete_post_meta($listing_id, 'wpcf-availability-rent');
        delete_post_meta($listing_id, 'wpcf-availability-minimum-income');
        delete_post_meta($listing_id, 'wpcf-availability-income-limit');
        delete_post_meta($listing_id, 'wpcf-availability-type');
        delete_post_meta($listing_id, 'wpcf-availability-units-available');
        delete_post_meta($listing_id, 'wpcf-availability-accessible-units');
        delete_post_meta($listing_id, 'wpcf-availability-view-apply');
        ...
        foreach ($rows_data as $row) {
            ...
            $rent = isset($row['rent']) ? trim($row['rent']) : '';
            $minimum_income = isset($row['minimum_income']) ? trim($row['minimum_income']) : '';
            ...
            // This will add it if it doesn't exist (e.g., "Single Room Occupancy (SRO)")
            ...
            add_post_meta($listing_id, 'wpcf-availability-property', $listing_id);
            add_post_meta($listing_id, 'wpcf-availability-town', $town);
            add_post_meta($listing_id, 'wpcf-availability-bedrooms', $unit_type);
            add_post_meta($listing_id, 'wpcf-availability-rent', $rent_clean);
            add_post_meta($listing_id, 'wpcf-availability-minimum-income', $minimum_income_clean);
            add_post_meta($listing_id, 'wpcf-availability-income-limit', $income_limit);
            add_post_meta($listing_id, 'wpcf-availability-type', $type);
            add_post_meta($listing_id, 'wpcf-availability-units-available', $units_available_text); // Save full text, not just number
            add_post_meta($listing_id, 'wpcf-availability-accessible-units', $accessible_units);
            add_post_meta($listing_id, 'wpcf-availability-view-apply', esc_url_raw($view_apply_url));
            
            $total_available += $units_count;
        }
        
        // Update total available (for backward compatibility)
        update_post_meta($listing_id, 'wpcf-total-available-units', $total_available);
        update_post_meta($listing_id, '_listing_total_available_units', $total_available);
    ...
    public function run_migration() {
        ...
        foreach ($grouped as $property_name => $rows_data) {
            ...
            if (empty($listing_types) || !in_array('rental', $listing_types)) {
                // Add all units for this property to not_imported_units (not a rental)
                foreach ($rows_data as $row) {
                    $this->results['not_imported_units'][] = array(
                        'property' => $property_name,
                        'bedrooms' => isset($row['bedrooms']) ? $row['bedrooms'] : '',
                ...
                continue; // Skip non-rental listings
            }
            
            // Update available units (pass all rows for this property)
            if ($this->update_listing_available_units(
                $listing->ID,
                $rows_data
            )) {
